...
CREATE TABLE IF NOT EXISTS recon_pattern_registry (pattern_id TEXT PRIMARY KEY, pattern_name TEXT, pattern_type TEXT, pattern_version TEXT DEFAULT '1.0', pattern_rule_json TEXT, status TEXT, execution_mode TEXT, confidence_threshold REAL, approved_by TEXT, approved_at TEXT DEFAULT CURRENT_TIMESTAMP);
...
def seed_default_patterns(conn: sqlite3.Connection) -> None:
    for pattern_id, name, pattern_type, rule, status, mode, threshold in DEFAULT_PATTERNS:
        conn.execute("""INSERT OR IGNORE INTO recon_pattern_registry (pattern_id, pattern_name, pattern_type, pattern_version, pattern_rule_json, status, execution_mode, confidence_threshold, approved_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""", (pattern_id, name, pattern_type, "1.0", json.dumps(rule), status, mode, threshold, "system_seed"))
