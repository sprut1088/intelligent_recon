...
# Must be first — before any app-module import — so sub-module loggers are active immediately.
...
def run_ai_pass() -> dict:
    ...
    import os as _os
    # write to a file — works regardless of --reload subprocess stdio redirection
    _dbg = _os.path.join(_os.path.dirname(__file__), "..", "ai_pass_debug.log")
    ...
    try:
        _log("endpoint hit")
        logger.warning("AI full pass requested")
        _log("phase 1 triage starting")
        triage_result = run_ai_triage()
        triaged = triage_result.get("inserted_count", 0)
        _log(f"phase 1 done triaged={triaged}")
        logger.warning("AI full pass: triage=%d", triaged)
        _log("phase 2 payment verify starting")
        verify_results = verify_exception_cases()
        verified = len(verify_results)
        _log(f"phase 2 done verified={verified}")
        logger.warning("AI full pass: verified=%d", verified)
        ...
        logger.exception("AI full pass failed: %s", exc)
